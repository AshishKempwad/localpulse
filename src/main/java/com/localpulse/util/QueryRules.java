...
import static com.localpulse.util.ElasticsearchUtil.*;
...
public class QueryRules {
    ...
    public static final QueryRule STATE_QUERY = QueryRule.of(
            srp-> Objects.nonNull(srp.state()),  // when state is present
            srp->buildTermQuery(STATE,srp.state(),1.0f)  // build term query
    );
